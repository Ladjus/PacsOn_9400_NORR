...
v.3: CTE cte_artnr_west: Select A/B-assortment plus C for PacsOn Norr.
v.4: Inkludera artiklar i Norr även om de inte finns i Falköping (5000), undo v.2. Remove INNER JOIN ars and WHERE-condition.
...
*/
    AND (ar_2000.q_saps4_sortiment IN ('A', 'B')  -- A/B-sortiment: alla artiklar. Change v.3.
         OR (ar_2000.q_saps4_sortiment IN ('C') AND ar_9400.artnr IS NOT NULL))  -- C-sortiment: endast artiklar som finns i Norr 9400. Change v.3.
),
...
--  AND ar_op.ForetagKod IN (9400)  -- Norr. Add v.1. Remove v.5.
...
*/
--  (ars.ForetagKod = 9400 AND ars.LagStalle IN ('5000') )  -- Norr. Add v.1. Remove v.4.
--  AND  -- Remove v.4.
