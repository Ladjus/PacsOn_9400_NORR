...
/* Change log
v.1: Inital version. Select only A-assortment plus B/C for PacsOn Norr. Add rows for reference-site/DC R310. Refer:
...
v.5: CTE cte_artnr_west: Select A/B-assortment plus C for PacsOn Norr.
...
/* Remove v.5.
    AND (ar_2000.q_saps4_sortiment NOT IN ('B', 'C')  -- Ej B/C-sortiment: alla artiklar.
         OR (ar_2000.q_saps4_sortiment IN ('B', 'C') AND ar_9400.artnr IS NOT NULL))  -- B/C-sortiment: endast artiklar som finns i Norr 9400.
*/
    AND (ar_2000.q_saps4_sortiment IN ('A', 'B')  -- A/B-sortiment: alla artiklar. Change v.5.
         OR (ar_2000.q_saps4_sortiment IN ('C') AND ar_9400.artnr IS NOT NULL))  -- C-sortiment: endast artiklar som finns i Norr 9400. Change v.5.
),
...
-- Purchasing
  CASE ars.ForetagKod  -- (smallint not nullable)
    WHEN 6000 THEN  -- Öst
      ...
      END
    WHEN 9400 THEN ars.InkHandl -- Norr: Map from Jeeves inköpshandläggare.
...
--  AND ar_op.ForetagKod IN (6000, 9400, 9400, 9500)  -- ÖVNS. Remove v.1.
--  AND ar_op.ForetagKod IN (9400)  -- Norr. Add v.1. Remove v.7.
...
FROM
  ...
  INNER JOIN ar AS ar_op  -- Operativa bolag
    ...
    AND ar_2000.ForetagKod = 2000  -- Mall
--  AND ar_op.ForetagKod IN (6000, 9400, 9400, 9500)  -- ÖVNS. Remove v.1.
    AND ar_op.ForetagKod IN (9400)  -- Norr. Add v.1.
  INNER JOIN ars
    ON ars.foretagkod = ar_op.foretagkod
    AND ars.artnr = ar_op.artnr
  LEFT OUTER JOIN al
